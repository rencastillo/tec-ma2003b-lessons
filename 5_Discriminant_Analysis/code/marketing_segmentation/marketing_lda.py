...
features = ['purchase_freq', 'avg_order_value', 'browsing_time', 'cart_abandonment',
...
print(f"LDA Cross-validation accuracy: {cv_scores_lda.mean():.3f} (+/- {cv_scores_lda.std() * 2:.3f})")
...
print(f"QDA Cross-validation accuracy: {cv_scores_qda.mean():.3f} (+/- {cv_scores_qda.std() * 2:.3f})")
...
for i, segment in enumerate(segments):
    ...
    plt.scatter(lda_scores_df.loc[mask, 'LD1'],
               ...
               c=colors[i], label=f'{segment} Customers',
               alpha=0.7, s=50)
...
plt.tight_layout()
plt.savefig(script_dir / "marketing_confusion_matrices.png", dpi=300, bbox_inches='tight')
plt.show()
